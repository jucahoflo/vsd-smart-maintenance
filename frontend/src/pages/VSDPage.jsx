...
const VSDPage = () => {
  ...
  const loadData = async () => {
    try {
      ...
      setVfds(data || []);
      console.log('📊 VSDs cargados:', data?.length || 0);
    } catch (error) {
  ...
  return (
    <Box>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          ...
          <Typography variant="body2" color="textSecondary">
            Gestión de VSDs y su estado operativo
          </Typography>
        </Box>
            ...
            <Card sx={{ 
              ...
              <CardContent>
                ...
                <Box mt={2} display="flex" justifyContent="flex-end" gap={1}>
                  <Button size="small" onClick={() => handleOpen(vfd)}>
                    <Edit fontSize="small" sx={{ mr: 0.5 }} /> Editar
                  </Button>
                  <Button size="small" color="error" onClick={() => handleDelete(vfd.id)}>
                    <Delete fontSize="small" sx={{ mr: 0.5 }} /> Eliminar
                  </Button>
                </Box>
              </CardContent>
            </Card>
          ...
          <Grid item xs={12}>
            <Card sx={{ borderRadius: 4, p: 4, textAlign: 'center' }}>
              ...
              <Typography variant="body2" color="textSecondary" mt={1}>
                Haz clic en "Nuevo VSD" para crear el primero
              </Typography>
            </Card>
          </Grid>
        )}
      ...
      <Dialog open={openDialog} onClose={handleClose} maxWidth="sm" fullWidth>
        <DialogTitle>
          <Typography variant="h6" fontWeight="700">
            {editing ? '✏️ Editar VSD' : '➕ Nuevo VSD'}
          </Typography>
        </DialogTitle>
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid item xs={12}>
              {editing ? (
                <TextField
                  ...
                  disabled
                  helperText="⚠️ El código NO se puede modificar una vez creado"
                  InputProps={{
                    readOnly: true,
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                placeholder="Ej: ABB, Siemens, Danfoss"
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField
